...
var sort  = {active:'sorted', asc : 'asc', desc : 'desc'}; 
...
cols.click(function (e) {
	...
	// $(cols).removeClass(sort.asc+' '+sort.desc); // remove icons from headers

	// if( $(this).data(sort.label) != sort.asc ){
	// 	tbody.append(rowsArray); // append sorted rows 
	// 	$(cols).data(sort.label, undefined); // reset cols to sort asc on first click
	// 	$(this).data(sort.label, sort.asc).addClass(sort.asc);
	// }
	// else{
	// 	tbody.append(rowsArray.reverse()); // append reversed rows
	// 	$(this).data(sort.label, sort.desc).addClass(sort.desc);
	// }
